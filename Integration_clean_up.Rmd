...
#make simpler gene activity matrix
...
#transfer.anchors <- FindTransferAnchors(reference =all.atac, query = all.rna, features = VariableFeatures(object = all.atac), reference.assay = "ACTIVITY", query.assay =  "RNA", reduction = "cca")
...
#imputation <- TransferData(anchorset = transfer.anchors, refdata = refdata, weight.reduction = all.rna[["pca"]])
#
...
library(ggplot2)
 library(mixtools)
#write out coembeded gene results
#coembed_final_standard_norm_singlet_filt.rds
