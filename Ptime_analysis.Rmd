...
RNA_ATAC_master_cl_added=data.frame(site_sep=character(),site=character(),log2fold=double(),pval=double(),celline=character(),gene=character(),corr_sp=double(),coaccess_orig=double(),qval=double(),coaccess_cl=character())
for (celline in levels(RNA_ATAC_master_up$celline)) {print(celline)
Combined_celline_site=data.frame(site=character(),coaccess=double())
...
List_linked_sites_cl<-Celline_up_sites[Celline_up_sites$gene==gene,] 
List_linked_sites_all<-RNA_ATAC_master_up[RNA_ATAC_master_up$gene==gene,] 
promoter=unique(List_linked_sites_all[List_linked_sites_all$coaccess==1,]$site)
...
{
print(gene)
pr_mod<-gsub(str_split_fixed(string = unique(promoter),pattern = "_CA_",2)[,1],pattern = "-",replacement = "_")
all_peak_linked_promoters <- (lapply(pr_mod,function(pr){cicero[cicero$Peak1==pr,]}) %>% bind_rows())
if (dim(all_peak_linked_promoters)[1]>0)
{
all_peak_linked_enhancers<-data.frame(RE=all_peak_linked_promoters$Peak2,coaccess=all_peak_linked_promoters$coaccess)
print(dim(all_peak_linked_enhancers))
#all_peak_linked_collapsed_enhancers<-
gene_collapsed<-aggregate(all_peak_linked_enhancers$coaccess,by=list(name=all_peak_linked_enhancers$RE),data=df,FUN=mean)

names(gene_collapsed)=c("site","coaccess")
Combined_celline_site<-rbind(Combined_celline_site,gene_collapsed)
}
else {print(paste0("no linked sites for: ", gene, "at: ", celline))}
}
else {print(paste0("no promoter for",gene))}
}
Master_up_combined<-merge(x = RNA_ATAC_master_up[RNA_ATAC_master_up$celline==celline,], Combined_celline_site, by.x="site_sep",by.y="site")
names(Master_up_combined)<-c("Site_sep","site","log2fold","pval","celline","gene","corr_sp","coaccess_orig","qval","coaccess_cl")
RNA_ATAC_master_cl_added<-rbind(Master_up_combined,RNA_ATAC_master_cl_added)
}

write.table(RNA_ATAC_master_cl_added,file="Celline_specific_coaccessibilty_up_0.001.txt",quote=F,sep="\t")

#plot this landscape as well


library(ggplot2)
g1<-ggplot()+geom_density_2d(data=RNA_ATAC_master_cl_added,aes(x=coaccess_cl,y=corr_sp,color=celline))+theme_classic()+scale_color_manual(values=colorcode)+theme(legend.position = "none",axis.title.x=element_blank(),axis.title.y=element_blank())+ facet_grid(celline ~ .) + geom_vline(xintercept = c(-0.15,0.15), linetype="dashed")+geom_hline(yintercept = c(-0.5,0.5), linetype="dashed")
...
#g2<-ggplot()+geom_density_2d(data=corr_fin_down,aes(x=coaccess,y=corr_sp,color=celline))+theme_classic()+scale_color_manual(values=colorcode)+theme(legend.position = "none",axis.title.x=element_blank(),axis.title.y=element_blank())+ facet_grid(celline ~ .) + geom_vline(xintercept = c(-0.15,0.15), linetype="dashed")+geom_hline(yintercept = c(-0.5,0.5), linetype="dashed")+geom_point(data=df_examples,aes(x=coaccess,y=corr_sp))+geom_text(data=df_examples,aes(x=coaccess,y=corr_sp,label=label),hjust=0, vjust=0)
#g<-ggarrange(g1,g2,widths=c(8,8),ncol=2,nrow=1)
ggsave(g1,filename = "Output_quartiles_up_cl_specific.pdf",width = 16,height = 26.4,units = "cm")
...
unique(RNA_ATAC_master_up[RNA_ATAC_master_up$gene=="AHNAK2",]$site)

#select for promoters

Up_promoters<-gsub(str_split_fixed(string = unique(RNA_ATAC_master_up_p$site),pattern = "_CA_",2)[,1],pattern = "-",replacement = "_")

sapply(Up_promoters,function(x){HCC1806_cicero[HCC1806_cicero$Peak1==x,]})
