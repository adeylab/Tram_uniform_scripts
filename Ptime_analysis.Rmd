...
#from here not the cl specific coaccess 
...
#how many
dim(final_ordered_difference_up[final_ordered_difference_up$difference_log2>1 & final_ordered_difference_up$difference_corr>1,])
123
...
dim(final_ordered_difference_down[final_ordered_difference_down$difference_log2>1 & final_ordered_difference_down$difference_corr>1,])
...
Enrichment of cell line specific sites
...
#this is when we test just within the cl specific sites
res_within_cl=data.frame(cl=character(),p=double())
#this is when we test within all unique cl sites
res_between_cl=data.frame(cl=character(),p=double())
for (file in list.files(path = "./combine_cl/"))
{
print("This is for within cl sites")
...
print(paste(sep=" ",file,length(intersect_cl),length(peak_ref_sort),158041-length(peak_ref_sort),length(peak_ps_sort),phyper(length(intersect_cl),length(peak_ref_sort),158041-length(peak_ref_sort),length(peak_ps_sort),lower.tail = F)))
...
res_within_cl=rbind(res_within_cl,res_cl)

#now between
print("This is for all cl sites")
intersect_b_cl<-bedr(input = list(a = peak_ps_sort, b = peak_un_sort), method="intersect",params ="-sorted",verbose=F);
print(paste(sep=" ",file,length(intersect_b_cl),33099,158041-33099,length(peak_ps),phyper(length(intersect_b_cl),33099,158041-33099,length(peak_ps),lower.tail = F)))
res_b_cl=data.frame(cl=file,p=phyper(length(intersect_b_cl),33099,158041-33099,length(peak_ps),lower.tail = F))
res_between_cl=rbind(res_between_cl,res_b_cl)
}

res_within_cl$q<-qvalue(res_within_cl$p, fdr.level=0.01,pi0 = 1)$qvalues
res_between_cl$q<-qvalue(res_between_cl$p, fdr.level=0.01,pi0 = 1)$qvalues
write.table(res_within_cl,file="Celline_result.hypregeom.test.w.cl.txt",quote = F,sep = "\t")
write.table(res_between_cl,file="Celline_result.hypregeom.test.b.cl.txt",quote = F,sep = "\t")
